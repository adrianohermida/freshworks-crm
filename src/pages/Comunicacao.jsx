...
export default function Comunicacao() {
  ...
  const { data: escritorio } = useQuery({
    queryKey: ['escritorio-comunicacao-page'],
    queryFn: () => base44.entities.Escritorio.list()
  });

  const { data: cliente } = useQuery({
    queryKey: ['cliente-comunicacao', user?.email, escritorio?.[0]?.id],
    queryFn: async () => {
      if (!user || !escritorio?.[0]?.id) return null;
