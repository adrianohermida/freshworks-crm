...
export default function MeusProcessos() {
  ...
  const { data: processos = [], isLoading: loadingProcessos } = useQuery({
    ...
    queryFn: async () => {
      ...
      const todos = await base44.entities.Processo.filter({ escritorio_id: escritorio.id }, '-updated_date', 200);
      // Prioridade 1: vinculado ao cliente
      if (cliente?.id) {
        const vinculados = todos.filter(p => p.cliente_id === cliente.id);
        if (vinculados.length > 0) return vinculados;
      }
      // Prioridade 2: criado pelo user
      const criados = todos.filter(p => p.created_by === user?.email);
      if (criados.length > 0) return criados;
      // Prioridade 3: polo_ativo ou polo_passivo contém o nome do user
      const nome = user?.full_name?.toLowerCase() || '';
      if (nome) {
        return todos.filter(p =>
          p.polo_ativo?.toLowerCase().includes(nome) ||
          p.polo_passivo?.toLowerCase().includes(nome)
        );
      }
      return [];
    },
