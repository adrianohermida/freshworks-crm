...
export default function DeadlineFromMovement({ movement, processId }) {
  const [open, setOpen] = useState(false);
  const [formData, setFormData] = useState({
    title: `Prazo: ${movement?.nome_movimento || 'Movimentação'}`,
    deadline_date: '',
    description: ''
  });
  ...
  const createDeadlineMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setFormData({ title: `Prazo: ${movement?.nome_movimento}`, deadline_date: '', description: '' });
    }
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Criar Prazo da Movimentação</DialogTitle>
        </DialogHeader>
