...
import { Scale, Home, Sparkles, Users, CreditCard, Mail, Info, Briefcase, TrendingUp, LogOut, Settings as SettingsIcon, FileText, Calendar, MessageSquare, Shield, Plug, BarChart3, DollarSign, Target, User as UserIcon, Inbox } from "lucide-react";
...
const userNavigationItems = [
  ...
  { title: "Meus Documentos", url: createPageUrl("MeusDocumentos"), icon: FileText },
...
const adminNavigationItems = [
  ...
  { title: "Publicações", url: createPageUrl("Publicacoes"), icon: FileText },
  { title: "Documentos", url: createPageUrl("DocumentosAdmin"), icon: FileText },
  { title: "Balcão Virtual", url: createPageUrl("BalcaoVirtual"), icon: MessageSquare },
  { title: "Helpdesk", url: createPageUrl("HelpdeskUnificado"), icon: Mail },
...
export default function Layout({ children, currentPageName }) {
  ...
  const [escritorio, setEscritorio] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('escritorio-cache')) || null;
    } catch {
      return null;
    }
  });
  const conversasPendentes = useConversasPendentes();
  const isOnboarding = currentPageName === "OnboardingCliente";
  ...
  const clientPages = ["MeuPainel","MeusProcessos","MeusTickets","MinhasConsultas","MinhasFaturas","MeusDocumentos","MeuPlanoPagamento","MinhaAgenda","AgendarConsulta","MeuPerfil","TicketDetalhesCliente","ProcessoDetalhesCliente"];
  ...
  useEffect(() => {
    if (isPublicPage && !isClientPage && !currentUser) {
      const existingScript = document.querySelector('script[src="//eu.fw-cdn.com/10713913/375987.js"]');
      if (!existingScript) {
  ...
  }, [isPublicPage, isClientPage, currentUser]);

  useEffect(() => {
    if (!currentUser || escritorio) return;
    base44.entities.Escritorio.list()
      .then(result => {
        const data = result?.[0] || null;
        setEscritorio(data);
        localStorage.setItem('escritorio-cache', JSON.stringify(data));
      })
      .catch(error => console.error('Erro ao carregar escritório:', error));
  }, [currentUser, escritorio]);
