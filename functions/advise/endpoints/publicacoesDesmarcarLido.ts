...
Deno.serve(async (req) => {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/core/v1/publicacoes-clientes/desmarcar-lidos`,
      {
        method: 'PUT',
        headers: {
    ...
    if (!response.ok) {
      ...
      return Response.json({
        ...
        error: errorData.mensagem || 'Erro ao desmarcar publicações',
        detalhes: errorData
    ...
    try {
      await base44.entities.AuditTrailColaboracao?.create({
        ...
        tipoOperacao: 'desmarcar_publicacoes_lidas',
        descricao: `${itens.length} publicação(ões) desmarcada(s) como não lida(s)`,
    ...
    return Response.json({
      success: true,
      action: 'advise.publicacoes.desmarcarLido',
      data: {
        publicacoesDesmaracadas: itens.length,
        resposta: result,
        timestamp: new Date().toISOString()
      }
    });
  } catch (error) {
    console.error('DesmarcarPublicacaoLido error:', error);
    return Response.json({
      success: false,
      error: error.message
    }, { status: 500 });
